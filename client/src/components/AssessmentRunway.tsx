...
type PathOption = { id: RunwayPath; driverId: string; eyebrow: string; title: string; copy: string; cue: string };
type RunwayCopy = {
  ...
  noPathTitle: string; noPathCopy: string; gateOpen: string; gateLocked: string; broaderNote: string; fieldNote: string; paths: PathOption[];
};
...
const COPY: Record<Language, RunwayCopy> = {
  zh: {
    ...
    noPathTitle: "尚未选择起始路径", noPathCopy: "请选择一个客户真实在谈的问题。系统不会默认把所有能力、地区或价值塞进同一个案例。", gateOpen: "Base ROI 已解锁", gateLocked: "仅机会范围", broaderNote: "这条路径会把你带到完整 Value Library；它不假装已有 Freight Audit／Visibility 的统一 Discovery 问卷。", fieldNote: "范围越小，证据越可信。", paths: [
      { id: "audit", driverId: "04", eyebrow: "OTM / 04", title: "运费、发票与结算完整性", copy: "客户在谈发票错误、重复付款、费率偏差或审计覆盖不足。", cue: "从 Freight Audit 开始" },
      { id: "visibility", driverId: "05", eyebrow: "OTM / 05", title: "可视化、异常与服务保障", copy: "客户在谈运输不可视、状态查询、人工作业、延误或加急。", cue: "从 Visibility 开始" },
      { id: "broader", driverId: "01", eyebrow: "ONE ORACLE", title: "其他运输或贸易问题", copy: "客户需要讨论规划、承运商、网络、车队、合规或全球贸易能力。", cue: "从完整 Value Library 开始" },
    ],
  },
  en: {
    ...
    noPathTitle: "No starting path selected", noPathCopy: "Choose the real customer issue being discussed. The system will not place every capability, geography, or value claim into one case by default.", gateOpen: "Base ROI unlocked", gateLocked: "Opportunity only", broaderNote: "This path opens the full Value Library. It does not pretend that Freight Audit or Visibility already has a universal Discovery file for every capability.", fieldNote: "Smaller scope creates more credible evidence.", paths: [
      { id: "audit", driverId: "04", eyebrow: "OTM / 04", title: "Freight spend, invoice and settlement integrity", copy: "The customer is discussing invoice errors, duplicate payment, rate variance, or limited audit coverage.", cue: "Start with Freight Audit" },
      { id: "visibility", driverId: "05", eyebrow: "OTM / 05", title: "Visibility, exceptions and service assurance", copy: "The customer is discussing low shipment visibility, status checking, manual effort, delays, or expedites.", cue: "Start with Visibility" },
      { id: "broader", driverId: "01", eyebrow: "ONE ORACLE", title: "Another transport or trade issue", copy: "The customer needs to discuss planning, carriers, network, fleet, compliance, or global-trade capabilities.", cue: "Start with the full Value Library" },
    ],
  },
  es: {
    ...
    noPathTitle: "No hay ruta inicial seleccionada", noPathCopy: "Elija el problema real que se está discutiendo. El sistema no colocará por defecto cada capacidad, geografía o valor en un mismo caso.", gateOpen: "ROI base desbloqueado", gateLocked: "Solo oportunidad", broaderNote: "Esta ruta abre la biblioteca completa de valor. No finge que Freight Audit o Visibility ya tengan un archivo Discovery universal para cada capacidad.", fieldNote: "Un alcance menor crea evidencia más creíble.", paths: [
      { id: "audit", driverId: "04", eyebrow: "OTM / 04", title: "Integridad de gasto, facturas y liquidación", copy: "El cliente habla de errores de factura, pagos duplicados, variación de tarifa o cobertura de auditoría limitada.", cue: "Empezar con Freight Audit" },
      { id: "visibility", driverId: "05", eyebrow: "OTM / 05", title: "Visibilidad, excepciones y garantía de servicio", copy: "El cliente habla de poca visibilidad, consultas de estado, esfuerzo manual, retrasos o urgencias.", cue: "Empezar con Visibility" },
      { id: "broader", driverId: "01", eyebrow: "ONE ORACLE", title: "Otro problema de transporte o comercio", copy: "El cliente necesita hablar de planificación, carriers, red, flota, cumplimiento o capacidades de comercio global.", cue: "Empezar con la biblioteca completa" },
    ],
  },
};
  ...
  return <section className="assessment-runway section-wrap section-anchor" id="assessment-runway">
    ...
    <div className="runway-problem-zone">
      ...
      <div className="runway-path-cards">
        {c.paths.map((option) => <button key={option.id} type="button" className={`runway-path-card ${path === option.id ? "active" : ""}`} onClick={() => onChoosePath(option.id)} aria-pressed={path === option.id}>
          <span className="runway-path-code">{option.eyebrow}</span><h3>{option.title}</h3><p>{option.copy}</p><div><span>{path === option.id ? c.current : option.cue}</span><ArrowRight size={16} strokeWidth={1.8} /></div>
        </button>)}
      </div>
    </div>
