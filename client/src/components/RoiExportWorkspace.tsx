...
const LAYER_COPY: Record<Language, { baseBenefit: string; opportunityBenefit: string; baseRoi: string; notReady: string; included: string; excluded: string; selectedCopy: string; disclaimer: string }> = {
  zh: { baseBenefit: "已验证价值", opportunityBenefit: "待确认机会价值", baseRoi: "首年 ROI", notReady: "资料尚待确认", included: "已验证，可计入", excluded: "待确认，暂不计入", selectedCopy: "只有已验证的项目会计入 ROI；其他项目仅保留为机会范围。", disclaimer: "此工具将尚待确认的项目保留为机会范围，仅将已验证项目计入 ROI。对外承诺前仍须确认范围、币种、TCO 与去重归属。" },
  en: { baseBenefit: "Validated benefit", opportunityBenefit: "Opportunity pending confirmation", baseRoi: "First-year ROI", notReady: "Information still needs confirmation", included: "Validated and included", excluded: "Pending confirmation", selectedCopy: "Only validated items enter ROI; all others remain opportunity-only.", disclaimer: "This workspace keeps pending items in the opportunity range and includes only validated items in ROI. Confirm scope, currency, TCO, and overlap ownership before external commitment." },
  es: { baseBenefit: "Beneficio validado", opportunityBenefit: "Oportunidad pendiente de confirmar", baseRoi: "ROI del primer año", notReady: "Información pendiente de confirmar", included: "Validado e incluido", excluded: "Pendiente de confirmar", selectedCopy: "Solo los elementos validados entran en ROI; los demás permanecen como oportunidad.", disclaimer: "Este espacio mantiene los elementos pendientes en el rango de oportunidad e incluye solo los validados en ROI. Confirme alcance, moneda, TCO y propiedad de solapamiento antes de un compromiso externo." },
};
...
export default function RoiExportWorkspace({ drivers, evidenceGates, onEvidenceGateChange, brandMarkSrc }: RoiExportWorkspaceProps) {
  ...
  return (
    <section className="roi-export-section section-wrap section-anchor" id="roi-export">
      <div className="section-lead"><div><div className="eyebrow">ROI export workspace</div><h2 className="section-heading">{c.title}</h2></div><p className="section-intro">{c.intro}</p></div>
      ...
      <div className="roi-scenario-dossier">
        ...
        {scenarioComparison.length > 0 ? <div className="roi-comparison-grid"><div className="roi-comparison-heading"><span>{c.register}</span><b>{c.saved}</b></div>{scenarioComparison.map(({ scenario, benefit, firstYear, roi: scenarioRoi, payback }) => <article className={`roi-scenario-card ${activeScenarioId === scenario.id ? "active" : ""}`} key={scenario.id}><div className="scenario-card-top"><div><span>{formatScenarioStamp(scenario.savedAt)}</span><h4>{scenario.name}</h4></div><i>{activeScenarioId === scenario.id ? c.current : c.saved}</i></div><div className="scenario-card-metrics"><div><span>{c.benefit}</span><strong>{currency.format(benefit)}</strong></div><div><span>{c.roi}</span><strong>{scenarioRoi.toFixed(1)}%</strong></div><div><span>{c.payback}</span><strong>{payback ? `${payback.toFixed(1)} ${c.months}` : "—"}</strong></div></div><p>{c.firstYear} {currency.format(firstYear)}</p><div className="scenario-card-actions"><button onClick={() => loadScenario(scenario)}>{c.load}</button><button onClick={() => removeScenario(scenario)}>{c.delete}</button></div></article>)}</div> : <div className="roi-empty-scenario"><span>{c.register}</span><p>{c.empty}</p></div>}
      </div>
      <div className="roi-workspace">
        <div className="roi-selection-panel">
          <div className="roi-panel-heading"><div><span>{c.selectLabel}</span><h3>{c.select}</h3></div><div className="roi-heading-mark"><img src={brandMarkSrc} alt="" /></div></div>
          ...
          <div className="roi-driver-list">
            {drivers.map((driver) => {
              ...
              return <div className={`roi-driver-row ${driver.status} ${driver.family.toLowerCase()} ${checked ? "checked" : ""}`} key={driver.id}>
                ...
                <div className="roi-driver-name"><div><strong><i className={`roi-family-chip ${driver.family.toLowerCase()}`}>{driver.family}</i>{driver.title}</strong><small>{driver.statusLabel} · {driver.impact}</small></div></div>
        ...
        <div className="roi-summary-panel">
          ...
          <div className="roi-gate-ledger"><div><span>{layer.baseBenefit}</span><strong>{currency.format(baseBenefit)}</strong><small>{layer.included}</small></div><div><span>{layer.opportunityBenefit}</span><strong>{currency.format(opportunityBenefit)}</strong><small>{layer.excluded}</small></div></div>
      ...
      <div className="roi-pdf-export" ref={pdfRef} aria-hidden="true">
        ...
        <section><span className="pdf-label">Selected value drivers</span>{selectedDrivers.map((driver) => <div className="pdf-driver" key={driver.id}><div><strong>{driver.title}</strong><span>{driver.statusLabel} · {driver.impact}</span></div><em>{currency.format(safeValue(driverValues[driver.id] ?? 0))}</em></div>)}</section>
