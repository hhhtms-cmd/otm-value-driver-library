      ...
      <main className="page-main">
        ...
        <section className="section-wrap section-anchor" id="library">
          ...
          <article className="detail-drawer" id="driver-detail" aria-live="polite">
            <div><div className="detail-file-strip"><img src={assetUrl("/manus-storage/otm-evidence-mark_3295b18f.png")} alt="" /><span>{selected.family} / {selected.statusLabel}</span><b className={selected.status}>{selected.statusLabel}</b></div><div className="detail-kicker">{selected.family} · {selected.statusLabel}</div><h3>{selected.title}</h3><p>{selected.narrative}</p><div className="path-line"><span className="path-step">{language === "zh" ? "你的运营问题" : language === "es" ? "Su problema operativo" : "Your operating question"}</span><i>→</i><span className="path-step">{selected.family} {familyCopy.capability}</span><i>→</i><span className="path-step">{t("经营变量")}</span><i>→</i><span className="path-step">KPI</span><i>→</i><span className="path-step">{t("价值判断")}</span></div></div>
