...
const SIMPLE_COPY = {
  zh: { top: "你的运输探索", kicker: "从一个运输问题开始", title: "先看清你最想改善的运输问题。", copy: "不用先理解 OTM，也不用准备一堆资料。选一个真实问题，我们帮你把下一步理清。", start: "开始理清我的问题", deeper: "深入查看完整工具", deepTitle: "已经想进一步了解？", deepCopy: "完整价值库、资料准备方法与 ROI 工具都在这里；只在你需要时打开。", back: "返回简单探索", other: "其他运输或贸易问题" },
  en: { top: "your transport exploration", kicker: "start with one transport question", title: "First, clarify the transport issue you most want to improve.", copy: "You do not need to understand OTM or prepare a lot of information first. Choose one real issue and we will make the next step clear.", start: "Clarify my question", deeper: "View the full toolkit", deepTitle: "Ready to look deeper?", deepCopy: "The full value library, information method, and ROI tools are here when you need them.", back: "Return to simple exploration", other: "Another transport or trade question" },
  es: { top: "su exploración de transporte", kicker: "empiece con una pregunta de transporte", title: "Primero, aclare el problema de transporte que más quiere mejorar.", copy: "No necesita entender OTM ni preparar mucha información primero. Elija un problema real y aclararemos el siguiente paso.", start: "Aclarar mi pregunta", deeper: "Ver el kit completo", deepTitle: "¿Listo para profundizar?", deepCopy: "La biblioteca de valor, el método de información y las herramientas ROI están aquí cuando las necesite.", back: "Volver a la exploración simple", other: "Otra pregunta de transporte o comercio" },
} as const;
...
export default function Home() {
  ...
  const simple = SIMPLE_COPY[language];
  ...
  return (
    <div className={`archive-shell ${advancedOpen ? "archive-deep-open" : "archive-simple"}`}>
      <div className="top-ledger"><span className="ledger-identity"><img src={assetUrl("/manus-storage/otm-evidence-mark_3295b18f.png")} alt="" />{advancedOpen ? "One Oracle Decision Archive · Your OTM Exploration" : simple.top}</span><div className="top-ledger-tools">{advancedOpen && <a href="/client-brief">{language === "zh" ? "三分钟快速探索" : language === "es" ? "Exploración rápida" : "Three-minute quick start"}</a>}<LanguageSwitcher />{advancedOpen && <span>Evidence before assertion</span>}</div></div>
      ...
      <main className="page-main">
        <section className="hero" id="overview">
          ...
          <div className="hero-content">
            ...
            <div className="hero-actions"><a className="button-archive" href="#workflow">{simple.start} <span>↓</span></a><button className="button-archive ghost" type="button" onClick={() => openAdvanced()}>{simple.deeper} <span>↗</span></button></div>
        ...
        <AssessmentWorkflow brandMarkSrc={assetUrl("/manus-storage/otm-evidence-mark_3295b18f.png")} onOpenAdvanced={() => openAdvanced()} />
        <section className="deep-entry section-wrap"><div><div className="eyebrow">{simple.deeper}</div><h2>{simple.deepTitle}</h2><p>{simple.deepCopy}</p></div><button type="button" onClick={() => openAdvanced()}>{simple.deeper} <span>↗</span></button></section>
        {advancedOpen && <div className="advanced-workbench" id="advanced-tools"><div className="deep-mode-bar"><span>{simple.deeper}</span><button type="button" onClick={() => { setAdvancedOpen(false); window.setTimeout(() => scrollToId("workflow"), 10); }}>← {simple.back}</button></div><AssessmentRunway language={language} path={runwayPath} stage={runwayStage} selectedDriver={selected} evidenceGate={evidenceGates[selectedId] ?? "E0"} onChoosePath={chooseRunwayPath} onOpenGtmModule={openGtmModule} onGoToDriver={goToRunwayDriver} onGoToDiscovery={goToRunwayDiscovery} onGoToEvidence={goToRunwayEvidence} onGoToRoi={goToRunwayRoi} onGoToOutput={goToRunwayOutput} />
