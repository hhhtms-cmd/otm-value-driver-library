      ...
      <main className="page-main">
        <section className="hero" id="overview">
          ...
          <div className="hero-content">
            ...
            {advancedOpen && <div className="hero-causal-rail" aria-label="Value causal path"><span>{t("痛点")}</span><b aria-hidden="true" /><span>{t("能力")}</span><b aria-hidden="true" /><span>{t("变量")}</span><b aria-hidden="true" /><span>KPI</span><b aria-hidden="true" /><span>{t("价值")}</span></div>}
            ...
            {advancedOpen && <div className="hero-runway-launch" aria-label={language === "zh" ? "从你的问题开始" : language === "es" ? "Empiece con su pregunta" : "Start with your question"}>
              ...
              <div className="hero-runway-launch-options">
                <button onClick={() => { chooseRunwayPath("optimization"); window.setTimeout(() => scrollToId("assessment-runway"), 10); }}><strong>{language === "zh" ? "运输计划与装载优化" : language === "es" ? "Optimización de envíos y cargas" : "Shipment Optimization"}</strong><i>↘</i></button>
                <button onClick={() => { chooseRunwayPath("audit"); window.setTimeout(() => scrollToId("assessment-runway"), 10); }}><strong>{language === "zh" ? "运费、发票与结算" : language === "es" ? "Gasto de flete, facturas y liquidación" : "Freight spend, invoices and settlement"}</strong><i>↘</i></button>
                <button onClick={() => { chooseRunwayPath("visibility"); window.setTimeout(() => scrollToId("assessment-runway"), 10); }}><strong>{language === "zh" ? "可视化与异常" : language === "es" ? "Visibilidad y excepciones" : "Visibility and exceptions"}</strong><i>↘</i></button>
                <button onClick={() => { chooseRunwayPath("broader"); window.setTimeout(() => scrollToId("assessment-runway"), 10); }}><strong>{language === "zh" ? "其他运输或贸易问题" : language === "es" ? "Otra pregunta de transporte o comercio" : "Another transport or trade question"}</strong><i>↘</i></button>
              </div>
            </div>}
          </div>
        </section>
        ...
        {advancedOpen && <div className="advanced-workbench" id="advanced-tools"><div className="deep-mode-bar"><span>{simpleCopy.deepNote}</span><div className="deep-mode-actions"><button className="deep-directory-jump" type="button" onClick={() => scrollToId("library")}>{language === "zh" ? "价值目录" : language === "es" ? "Biblioteca de valor" : "Value library"}</button><button type="button" onClick={() => { setAdvancedOpen(false); window.setTimeout(() => scrollToId("workflow"), 10); }}>← {simpleCopy.returnSimple}</button></div></div><AssessmentRunway language={language} path={runwayPath} stage={runwayStage} selectedDriver={selected} evidenceGate={evidenceGates[selectedId] ?? "E0"} onChoosePath={chooseRunwayPath} onOpenGtmModule={openGtmModule} onGoToDriver={goToRunwayDriver} onGoToDiscovery={goToRunwayDiscovery} onGoToEvidence={goToRunwayEvidence} onGoToRoi={goToRunwayRoi} onGoToOutput={goToRunwayOutput} />
        ...
        <section className="narrative-section section-wrap section-anchor" id="narrative">
          ...
          <div className="narrative-grid"><div className="narrative-steps"><div className="narrative-file-strip"><img src={assetUrl("/manus-storage/otm-evidence-mark_3295b18f.png")} alt="" /><span>CASE FILE / SCOPE CONTROL</span></div><div className="narrative-step"><span className="step-number">CORE</span><div><h3>{t("已验证的核心价值")}</h3><p>{t("US freight audit 与 visibility 的直接价值闭环；使用基线、范围、去重规则和敏感性表达。")}</p></div><span className="step-tag">Hard ROI</span></div><div className="narrative-step"><span className="step-number">EXTEND</span><div><h3>{t("有证据、待量化的扩展价值")}</h3><p>{t("Europe 的相同 driver 或相邻机会；说明诊断和验证计划，不承诺具体金额。")}</p></div><span className="step-tag">Validate</span></div><div className="narrative-step"><span className="step-number">FUTURE</span><div><h3>{t("战略路线图选项")}</h3><p>{t("网络建模、车队、可持续性或更广 Oracle 能力；保留给独立的范围确认和商业案例。")}</p></div><span className="step-tag">Roadmap</span></div></div><div className="narrative-art"><img src={assetUrl("/manus-storage/otm-narrative-layers_d2d229db.jpg")} alt="Abstract layered business-case narrative" /></div></div>
        </section>
