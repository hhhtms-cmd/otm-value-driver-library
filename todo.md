...
- [x] 审计默认首页的目录、内部编号、ROI 与完整资料库，划分客户默认与深入内容。
- [x] 设计极简首页的客户起点、深入查看入口与一键保存团队讨论清单体验。
- [x] 实现默认隐藏的完整目录与 ROI 深入入口，并让每条下一步可保存为团队讨论清单。
- [x] 验证桌面手机端的默认路径、保存清单和深入入口，并发布版本。
- [x] 定位首页主标题左侧竖线装饰的样式来源。
- [x] 仅移除该竖线，保留网格、标题、文案和按钮构图不变。
- [x] 验证最小视觉修正并发布版本。
- [x] 审计完整 Value Driver 与 ROI 工作台中暴露的内部编号、阶段编号和无关标签。
- [x] 移除客户可见的跳号编号，保留主题名称、选择动作、后台关联与 ROI 计算。
- [x] 验证完整工具无跳号困惑、ROI 选择计算正常，并发布版本。
- [x] 审计当前答案结果与现有任务组件，定义客户探索的结果闭环。
- [x] 设计只含案例、参与人和三项讨论问题的团队讨论清单，并提供复制、下载和深入入口。
- [x] 实现答案后的讨论清单结果、复制下载与深入工具跳转。
- [x] 验证答案到结果的完整闭环、三语文案、手机体验与发布版本。
- [x] 审计完整目录的侧栏与价值域行在桌面端的实际列宽和换行原因。
- [x] 重新定义侧栏宽度，以及名称、说明、状态和进入动作的横向网格。
- [x] 修复完整目录布局，不改变首页极简探索与工具功能。
- [x] 验证桌面和手机端目录可读性、详情点击和发布版本。
- [x] 审计中等桌面宽度下侧栏、ROI、价值域列表与双栏内容的断点冲突。
- [x] 设计目录收纳和主内容单栏／堆叠规则，避免中文被压成竖排。
- [x] 实现深入工具中等宽度的响应式重构，不改变默认首页探索。
- [x] 验证目标宽度、手机和大屏的目录、ROI、问题区及发布版本。
