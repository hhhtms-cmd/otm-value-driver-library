...
- [x] 确认当前 GitHub 账户、项目远程关联和安全导出目标，避免覆盖现有仓库。
- [x] 创建专用私有仓库并推送当前 UI 源代码。
- [x] 验证仓库内容、记录访问链接并提供 Gemini 审查使用方式。
- [x] 将 GPT 的“客户问题优先、专家方法论后置”建议映射到当前首页与工作流。
- [x] 定义只含客户问题入口、简单选择和明确下一步的默认页面结构。
- [x] 重构默认 UI，隐藏内部术语、证据框架、价值库与 ROI，改为按需深入查看。
- [x] 验证中英西三语、桌面手机端与完整工具入口，并发布版本。
